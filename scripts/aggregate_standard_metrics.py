...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate standard effect metrics from Stage-B artifacts.")
    ...
    try:
        ...
        output = {
            **aggregate,
            ...
            "skipped": skipped,
            "groups": group_standard_metrics(metrics, args.group_by) if args.group_by else [],
            "benign_drop_by_defense": compute_benign_drop_by_defense(metrics),
        }
        if args.output:
        ...
        if args.json:
            ...
            print(f"Standard metric aggregation failed: {type(exc).__name__}: {exc}", file=sys.stderr)
        return 1
